...
fut = asyncio.Future() # это создание объекта Future из библиотеки asyncio. Future представляет собой результат асинхронной операции, который станет доступен в будущем. 
...
# В Python корутина (Coroutine) – это особая функция, которая может приостанавливать свое выполнение и возобновлять его позже, как будто она выполняет несколько задач одновременно. Это ключевое понятие в асинхронном программировании. 